## v2.3.0
 ...
 - The `route.stub` file have been modified.
  ...
  - The user is no longer required to publish the default templates. This is much better step to prevent upgrade braking when the template are updated during a patch release. The user should publish templates only if he/she want to modify it and rename it. It is best to delete existing template and template-collective from the folders from the default publish path of your project.
  - The user is no longer have to publish resource to install the package! One line only is required to install the package on laravel 5.5+ (i.e, composer require crestapps/laravel-code-generator --dev)
